    ...
    (row) => (
      <div className="@flex @space-x-2 smallscreen:@space-x-4">
        <Image
          ...
          alt={row.army}
        />
        <Link className="@p-0 @m-0 @text-white hover:@text-primary @text-[1em]" href={row.profileLink}>
          {row.name}
        </Link>
      </div>
    ),
  ...
  columnHelper.accessor("winRate", {
    ...
    cell: (info) => (
      <div className="@relative">
        <div className="@text-left @pl-4 @relative @w-full @h-full @z-10">{info.getValue().toFixed(2)} %</div>
        <div
