...
export default function LeaderboardFilters({ gamemode, timeMode, setGamemode, setTimeMode }: Props) {
  return (
    <div className="@grid @gap-6 @grid-cols-2 smallscreen:@grid-cols-3 laptop:@gap-4 laptop:@grid-cols-4 monitor:@grid-cols-6 @mb-8 smallscreen:@mb-12">
      <div className="@w-30 cellphone:@w-40 tablet:@w-56 large_monitor:@w-80 @space-y-2">
        ...
        <Select options={gamemodes} value={gamemode} onChange={(o) => setGamemode(o)} />
      </div>
      <div className="@w-30 cellphone:@w-40 tablet:@w-56 large_monitor:@w-80 @space-y-2">
        <label>Time mode</label>
        <Select options={timeModes} value={timeMode} onChange={(o) => setTimeMode(o)} />
      </div>
    </div>
  );
}
