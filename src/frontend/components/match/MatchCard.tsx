...
export default function MatchCard({
  ...
  return (
    <div className="@flex @flex-col @bg-black/50 @my-4 @shadow-black/60 @cursor-pointer hover:@scale-105 @transition @h-[300px] @w-[600px]">
      <div id="mapBar" className="@flex @flex-grow-0 @flex-shrink-0 @h-8 @items-center @bg-bg-tertiary @shadow-md">
        ...
        <div className="@relative @flex @flex-col @justify-between @flex-grow @flex-shrink @z-10">
          ...
          <div id="coImages" className="@flex @justify-between @h-[200px]">
            <div className="@flex @flex-col @overflow-hidden @pl-4 @items-end">
              <img className="@transform @scale-x-[-1] @max-w-[200px] @object-contain"  src={`/img/CO/smoothFull/Awds-${co1}.webp`}/>
            </div>
            <div className="@flex @flex-col @overflow-hidden @pr-4 @items-end">
              <img className="@max-w-[200px] @object-contain" src={`/img/CO/smoothFull/Awds-${co2}.webp`}/>
            </div>
          </div>
        </div>
        <div id="playerRow" className="@h-8 @z-10">
          <div className="@flex @h-full @justify-between @self-end @gap-2">
            <PlayerNameBar name={playerMatches[0]?.name} rank={randomRank} armyIndex={armyIndex} iconSide={SideEnum.Left}/>
            <PlayerNameBar name={playerMatches[1]?.name} rank={randomRank2} armyIndex={armyIndex2} iconSide={SideEnum.Right}/>
          </div>
        </div>        
      </div>
    </div>
