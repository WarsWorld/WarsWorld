...
export const jessAW2: COProperties = {
  ...
  gameVersion: "AW2",
  dayToDay: {
    description: "Ground vehicles have +10% firepower. Other units (including footsoldiers) have -10% firepower.",
    hooks: {
      attack: ( {attacker} ) => {
        if (attacker.properties().facility === "base" && attacker.data.type !== "infantry" && attacker.data.type !== "mech") {
          return 110;
        }
