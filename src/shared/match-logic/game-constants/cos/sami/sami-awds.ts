...
export const samiAWDS: COProperties = {
  ...
  dayToDay: {
    description:
      "Footsoldiers have +20% firepower and capture at 1.5 times the normal rate (rounded down). Other direct units have -10% firepower.",
    hooks: {
      attack: ({ attacker }) => {
  ...
  powers: {
    ...
    superCOPower: {
      name: "Victory March",
      description:
        "Footsoldiers gain 2 movement, +60% firepower and the ability to capture instantly.",
      stars: 8,
      hooks: {
        movementPoints: (points, unit) => {
          if (unit.isInfantryOrMech()) {
